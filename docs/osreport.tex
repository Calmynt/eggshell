...
\begin{document}
    ...
    \chapter{Testing the \ii{eggshell}}
        \section{\tx{./eggshell test}}
            The main file that uses the eggshell has support for this additional
            parameter when running the program. If the \tx{test} argument is 
            detected when running \tx{eggshell}, the program will immediately
            \tx{source testinput.txt}, running it, before exiting. 

            The file mentioned contains multiple commands, testing almost every
            aspect of the \tx{eggshell} apart from the signal handling. This is 
