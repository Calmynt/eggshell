    ...
    \chapter{Code documentation}
        ...
        \section{\tx{variables.c}}
            ...
            \mysubsect{\tx{int parse\_var}}{\tx{(char *line)}}
                ...
                If any of these checks fail, the function returns \tx{1}, meaning
                that a valid assignment wasn't made, meaning the main function should
                keep on attempting to parse the line. There is only a single exception,
                which is if the \tx{=} is surrounded by whitespace. There is no command
                that requires a stray \tx{=}, and such a situation causes problems with
                the rest of the \tx{execute} parser, so in this case, the function returns
                \tx{0} to stop parsing, while setting the exitcode accordingly.
